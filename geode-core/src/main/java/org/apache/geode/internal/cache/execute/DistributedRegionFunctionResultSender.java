...
public class DistributedRegionFunctionResultSender implements InternalResultSender {
  ...
  public void lastResult(Object oneResult) {
    ...
    if (this.sender != null) { // Client-Server
      ...
      if (isLocal) {
        ...
        this.rc.endResults();
        FunctionStatsImpl.getFunctionStats(functionObject.getId(), this.dm.getSystem())
            .incResultsReceived();
      } else {
        try {
  ...
  public void lastResult(Object oneResult, DistributedMember memberID) {
    ...
    if (this.sender != null) { // Client-Server
      ...
      // incrementing result sent stats.
      if (this.dm == null) {
        FunctionStatsImpl.getFunctionStats(functionObject.getId()).incResultsReceived();
      } else {
        FunctionStatsImpl.getFunctionStats(functionObject.getId(), this.dm.getSystem())
            .incResultsReceived();
      }
  ...
  public synchronized void sendResult(Object oneResult, DistributedMember memberID) {
    ...
    if (this.sender != null) { // Client-Server
      ...
      // incrementing result sent stats.
      if (this.dm == null) {
        FunctionStatsImpl.getFunctionStats(functionObject.getId()).incResultsReturned();
      } else {
        FunctionStatsImpl.getFunctionStats(functionObject.getId(), this.dm.getSystem())
            .incResultsReturned();
      }
